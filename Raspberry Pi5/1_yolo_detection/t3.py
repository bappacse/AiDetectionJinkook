...
my_file = open("microfluidic_detection.txt", "r")
data = my_file.read()
...
while True:
    ...
    # 调整图像大小
    resized_image = cv2.resize(target_image1, (160, 112))
    count += 1
    ...
    for index,row in px.iterrows():
        ...
        cv2.rectangle(target_image1,(x1*4,y1*4),(x2*4,y2*4),(0,0,255),2)
        if d == 0:
            cvzone.putTextRect(target_image1,f'{c}, {conf_score:.2f}',(x1*4,y1*4-30),1,1)
        elif d == 1:
            cvzone.putTextRect(target_image1,f'{c}, {conf_score:.2f}',(x1*4,y1*4-40),1,1)
